...
function animateSlow(obj, attr, over) {
    ...
    obj.timerid = setInterval(function () {
        ...
        for (var k in attr) {
            if (k == 'opacity') {
                var number = 100;
                var set = parseFloat(getStyle(obj, k)) * number;
                var garget = attr[k] * number;
                var get = (garget - set) / 10;
                var num = get > 0 ? Math.ceil(get) : Math.floor(get);
                set += num;
                obj.style[k] = set / number;
                if (set != garget) {
                    switc = false;
                }
            } else {
                var set = parseInt(getStyle(obj, k));
                var garget = attr[k];
                var get = (garget - set) / 10;
                var num = get > 0 ? Math.ceil(get) : Math.floor(get);
                set += num;
